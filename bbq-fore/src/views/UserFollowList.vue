<template>
  <div class="mcontaner">
    <Header></Header>
    <h1 style="text-align: center">your follow</h1>
    <div class="block">
      <el-table
          :data="follows"
          stripe
        ...
        <el-table-column label="username" width="160">
          <template slot-scope="scope">
            <router-link :to="{name: 'UserIndex', params: {userId: scope.row.followedInfo.id}}">
              {{scope.row.followedInfo.username}}
            </router-link>
          </template>
        </el-table-column>
        <el-table-column label="avatar">
          <template slot-scope="scope">
            <el-avatar :size="60" :src="scope.row.followedInfo.avatar"></el-avatar>
          </template>
        </el-table-column>
        <el-table-column
            prop="followedInfo.sex"
            label="sex">
        </el-table-column>
        <el-table-column
            prop="followedInfo.signature"
            label="signature">
        </el-table-column>
        <el-table-column
            prop="followedInfo.role"
            label="role">
        </el-table-column>
        <el-table-column label="operator" width="130">
          <template slot-scope="scope" >
            <span>
            <el-button size="mini" type="primary" plain @click = "unfollow(scope.row.followedInfo.id)">delete</el-button>
            </span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    page(currentPage) {
      ...
      _this.$axios.get("/user/followed/list?userId="+this.$store.getters.getUser.id+"&pageNum=" + currentPage)
          .then(res => {
